...
async def download_video(event):
    ...
    if len(a) >= 5 and a[5] == "s":
        return
    await event.edit("__memproses musik, tunggu sebentar...__")
    url = event.pattern_match.group(1)
    if not url:
        return await event.edit("**List Error**\nGunakan : -`.musik <Judul Lagu>`")
    ...
    try:
        ...
        return await event.edit("`musik tidak ditemukan...`")
    type = "audio"
    await event.edit(f"`Bersiap mengunduh {url}...`")
    if type == "audio":
        opts = {
            "format": "bestaudio",
            "addmetadata": True,
...
async def download_vsong(event):
    ...
    try:
        ...
        return await x.edit("`tidak ada lagu yang ditemukan...`")
    type = "audio"
...
async def original(event):
    if not event.pattern_match.group(1):
        return await event.edit("Berikan judul untuk mencari lirik nya.\n**Contoh** : `.lirik` <Judul Lagu>")
...
CMD_HELP.update(
    {
        "songdl": "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.song <Artis - Judul Lagu>`\
         \n↳ : Unduh lagu yang kamu inginkan.\
